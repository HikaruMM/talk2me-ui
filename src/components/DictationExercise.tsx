...
import { 
  ...
  Check, AlertTriangle, Lightbulb, Volume2, HelpCircle, 
  CheckSquare, Square, Layers, Repeat, Maximize2, FastForward,
...
export const DictationExercise: React.FC<DictationExerciseProps> = ({
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      // Don't intercept if user is typing inside textarea unless it's specific hotkeys
      if (e.key === 'Tab') {
        e.preventDefault();
        speakSegment();
  ...
  return (
    <div className="w-full space-y-6">
      ...
      {/* 3-COLUMN DICTATION WORKSPACE CONTAINER */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        
        {/* LEFT COLUMN (3 cols): CAPTIONS LIST & LEVEL SELECTOR & PROGRESS */}
        <div className="lg:col-span-3 space-y-4 bg-white dark:bg-[#1E293B] p-4 rounded-3xl border border-[#E4E8F0] dark:border-[#334155] shadow-sm">
          ...
          {/* Scrollable Caption Lines List */}
          <div className="space-y-2 max-h-[380px] overflow-y-auto pr-1">
            {segments.map((seg, idx) => {
              const isActive = activeSegmentIndex === idx;
              const isSubmitted = !!submittedStatuses[idx];
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="font-mono text-xs font-bold text-slate-800 dark:text-slate-200">
                        {showCaptions && isSubmitted ? seg.targetText : maskedText}
                      </span>
                    </div>
                  </div>
        ...
        {/* CENTER COLUMN (4 cols): VIDEO PLAYER & METADATA */}
        ...
        {/* RIGHT COLUMN (5 cols): EXPANDED DICTATION INPUT & ACTION CONTROLS & HOTKEYS */}
        <div className="lg:col-span-5 space-y-4">
          ...
          {/* Asterisk Representation Under Next Button */}
