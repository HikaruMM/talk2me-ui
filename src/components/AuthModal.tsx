...
import { X, Mail, Lock, User, AlertCircle, ArrowLeft, Check, Pencil, Eye, EyeOff } from 'lucide-react';
...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const handleContinueEmail = (e: React.FormEvent) => {
    ...
    // If initialMode was signup, default to signup step, otherwise password
      ...
      >
        ...
        {step === 'email' && (
          <div className="mt-5 space-y-4">
            ...
            {/* Social Logins matching exact screenshot styling */}
        ...
        {/* STEP 2: PASSWORD STEP (MATCHING EXACT SCREENSHOT 2) */}
        {step === 'password' && (
          <form onSubmit={handlePasswordSubmit} className="mt-5 space-y-4">
            
            {/* Email Field with Edit Pencil Icon */}
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-slate-800 dark:text-slate-200">
                Email <span className="text-red-500">*</span>
              </label>
            ...
            {/* Button 1: Tiếp Theo (Primary Solid Blue) */}
            <button
              type="submit"
              disabled={isLoading}
            ...
            {/* Button 2: Đăng Nhập Bằng Liên Kết (Secondary Outlined Blue) */}
            <button
              type="button"
            ...
            {/* Bottom Links: Đăng ký & Đăng nhập với tổ chức */}
            <div className="pt-2 text-center text-xs text-slate-600 dark:text-slate-400 space-y-2">
              ...
              <div>
                <button
                  type="button"
                  onClick={() => alert('Chức năng đăng nhập tổ chức (SSO) đang được chuẩn bị.')}
                  className="text-xs text-[#1D5BD8] dark:text-blue-400 hover:underline"
                >
                  Đăng nhập với tổ chức của bạn
                </button>
              </div>
            </div>
        ...
        {/* STEP 3: SIGNUP STEP (MATCHING SCREENSHOT 3) */}
        {step === 'signup' && (
          <form onSubmit={handleSignupSubmit} className="mt-5 space-y-4">
            ...
            {/* Primary Action Button: Tham Gia Miễn Phí */}
            <button
              type="submit"
              disabled={isLoading}
            ...
            {/* Link: Đã có tài khoản Coursera / Talk2Me? Đăng nhập */}
            <div className="pt-2 text-center text-xs text-slate-600 dark:text-slate-400 space-y-2.5">
              ...
              <div>
                <button
                  type="button"
                  onClick={() => alert('Chức năng đăng ký tổ chức (SSO) đang được chuẩn bị.')}
                  className="text-xs text-[#1D5BD8] dark:text-blue-400 hover:underline"
                >
                  Đăng ký thông qua tổ chức của bạn
                </button>
              </div>
            </div>
        ...
        {/* Footer Disclaimer & Links (Matching Screenshot) */}
        <div className="mt-6 pt-4 border-t border-slate-100 dark:border-slate-800 text-[11px] text-slate-500 dark:text-slate-400 space-y-2 leading-relaxed">
          {step === 'signup' ? (
            <p className="text-[10px] text-slate-400">
              Tôi chấp nhận{' '}
              <a href="#" className="text-slate-600 dark:text-slate-300 underline">Điều khoản Sử dụng</a>{' '}
              và{' '}
              <a href="#" className="text-slate-600 dark:text-slate-300 underline">Thông báo về quyền riêng tư</a>{' '}
              của Talk2Me. Gặp sự cố khi đăng nhập?{' '}
          ...
          <p className="text-[10px] text-slate-400">
            ...
            <a href="#" className="text-slate-500 underline">Chính sách bảo mật</a> và{' '}
            <a href="#" className="text-slate-500 underline">Điều khoản dịch vụ</a> của Google.
          </p>
        </div>
