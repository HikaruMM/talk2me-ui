...
import { INITIAL_FLASHCARD_FOLDERS, INITIAL_FLASHCARD_SETS } from '../data/mockFlashcards';
...
export const FlashcardDeck: React.FC<FlashcardDeckProps> = () => {
  ...
  const handleSaveSet = (newSet: FlashcardSet, practiceNow: boolean = false) => {
    ...
    // If assigned to a folder, update folder setIds
    if (newSet.folderId) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6 animate-in fade-in duration-200">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-2">
        ...
        {/* Right side controls: Search input + Create Button */}
        <div className="flex items-center gap-3">
          {/* Search Bar pill */}
      ...
      {/* SECTION 1: JUMP BACK IN (ÔN NHANH) - Fixed at Top */}
      {studySets.length > 0 && (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {studySets.slice(0, 2).map((set, idx) => (
              ...
              >
                {/* Background Decorative Card Graphics */}
      ...
      {/* SECTION 2: RECENTS (GẦN ĐÂY) - Fixed at Top */}
      <div className="space-y-4">
        <h2 className="text-xl font-black text-[#1B1F2E] dark:text-white tracking-tight">
      ...
      {/* FILTER BUTTONS ROW - Moved down next to filter area above library list */}
      ...
      {/* TAB CONTENT: STUDY SETS (HỌC PHẦN) */}
      {activeTab === 'sets' && (
        <div className="space-y-4">
          <h2 className="text-xl font-black text-[#1B1F2E] dark:text-white tracking-tight">
