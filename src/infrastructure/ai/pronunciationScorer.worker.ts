/**
 * PronunciationScorer Worker — ONNX Runtime Web + CTC Forced Alignment + GOP Scoring
 *
 * Runs entirely off the main thread — model inference + the pure-JS Viterbi forced-alignment
 * pass are CPU-heavy enough (single-threaded WASM, see numThreads=1 below) to freeze the UI
 * for the duration of a scoring call otherwise. See pronunciationScorer.ts for the main-thread
 * RPC wrapper (same protocol shape as ttsEngine.ts/ttsEngine.worker.ts).
 *
 * What stays on the main thread instead of here:
 * - Audio Blob → PCM decode/resample (audioProcessor.ts) — needs AudioContext, which does not
 *   exist in a Worker's global scope.
 * - Model byte download + CacheStorage read/write (resourceManager.ts) — its
 *   pronunciationDownloadStore is a plain in-memory singleton subscribed to by main-thread UI
 *   (ResourceManagerSection etc.); a copy of that module running inside the worker's separate
 *   realm would update its own disconnected instance and never reach the UI. Also, like
 *   ttsEngine.worker.ts, localStorage isn't available inside a Dedicated Worker.
 *
 * The already-downloaded model bytes are transferred in via the 'load-model' message instead.
 */

import * as ort from 'onnxruntime-web';
import { unzipSync } from 'fflate';
import { fetchWordPhonemes, type WordPhonemes } from './g2p';
...
/**
 * Build the ONNX inference session from already-downloaded model bytes (raw .onnx or a
 * zip containing one — handles nested zips too, same as the main-thread code this replaced).
 */
async function loadSession(modelBuffer: ArrayBuffer): Promise<ort.InferenceSession> {
  ort.env.wasm.wasmPaths = ORT_WASM_BASE_URL;

  // Multi-threaded WASM needs SharedArrayBuffer, which browsers only expose on a
  // cross-origin-isolated page (COOP/COEP headers — see server.ts and public/_headers).
  // Without that, SharedArrayBuffer doesn't exist and forcing numThreads > 1 would throw,
  // so this must check crossOriginIsolated rather than just using hardwareConcurrency.
  // Capped at 4: this model is small enough that more threads mostly add scheduling
  // overhead, and it leaves the other cores free for the UI thread / rest of the page.
  ...
  let onnxBuffer: Uint8Array = new Uint8Array(modelBuffer);
  ...
  const extracted = extractOnnx(onnxBuffer);
  if (extracted) {
  ...
  return ort.InferenceSession.create(onnxBuffer.buffer, opts);
}

// ─── Message protocol ─────────────────────────────────────

export type ToWorkerMessage =
  | { type: 'load-model'; buffer: ArrayBuffer }
  | { type: 'score'; requestId: number; pcm: ArrayBuffer; targetText: string };

export type FromWorkerMessage =
  | { type: 'model-loaded' }
  | { type: 'model-error'; message: string }
  | { type: 'score-done'; requestId: number; result: ShadowingResult }
  | { type: 'score-error'; requestId: number; message: string };
...
/**
 * Score pronunciation of already-decoded PCM audio against target text.
 *
 * Pipeline:
 * 1. ONNX Wav2Vec2 INT8 inference → logits
 * 2. Log-softmax → log probabilities
 * 3. CTC Forced Alignment (Viterbi) → char segments
 * 4. G2P → map chars to phonemes → GOP scoring
 * 5. Return ShadowingResult with per-word, per-phoneme scores
 */
async function scorePcm(pcm: Float32Array, targetText: string): Promise<ShadowingResult> {
  const t0 = performance.now();

  if (!cachedSession) {
    throw new Error('Model chưa được tải trong worker — gọi load-model trước.');
  }
  const session = cachedSession;

  // Kick off the phoneme-lookup network call immediately, in parallel with inference below —
  // it's independent of the model output, so overlapping it hides its network latency.
  const wordPhonemesPromise = fetchWordPhonemes(targetText);
  ...
  // 2b. Content-match guard: free decode (no target constraint) to check whether the
  ...
  // 3. Get char IDs from target text
  ...
  // 5. Map to phoneme scores (phoneme lookup was already fetched in parallel above)
  const wordPhonemes = await wordPhonemesPromise;
  const wordAnalysis = mapToPhonemeScores(segments, wordPhonemes, decodedTranscript);
...
ctx.addEventListener('message', async (event: MessageEvent<ToWorkerMessage>) => {
  ...
  if (msg.type === 'load-model') {
    try {
      cachedSession = await loadSession(msg.buffer);
  ...
  if (msg.type === 'score') {
    try {
      ...
      const result = await scorePcm(pcm, msg.targetText);
      post({ type: 'score-done', requestId: msg.requestId, result });
    } catch (err) {
      post({
        type: 'score-error',
